...
public class UploaddpActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_uploaddp);
    ...
    private void uploadImage(File fileImage){
        ...
        Call<Upload> call = apiInterface.uploadDpImage(bundle.getString("TRANSACTION_CODE"), image);
        call.enqueue(new Callback<Upload>() {
            @Override
            public void onResponse(Call<Upload> call, Response<Upload> response) {
                if (response.code() == 202){
                    Toast.makeText(getApplicationContext(),"Bukti DP berhasil di upload",Toast.LENGTH_SHORT).show();
                } else {
